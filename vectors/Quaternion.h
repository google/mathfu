...
class Quaternion {
 public:
  ...
  inline Quaternion<T> operator*(const T& s1) const {
    T angle; Vector<T, 3> axis;
    ToAngleAxis(&angle, &axis);
    angle*=s1;
    return Quaternion<T>(cos(.5 * angle), axis.Normalized()*sin(.5 * angle));
  }

  /// Quaternion/Vector multiplication. This will rotate the given vector by the
  /// rotation stored within the quaternion.
  ...
  inline Vector<T, 3> ToEulerAngles() {
    ...
    if (cos2 < 1e-6) {
      return Vector<T, 3>(
        0, m[2] < 0 ? .5 * M_PI : -.5 * M_PI, -atan2(m[3], m[4]));
    } else {
      return Vector<T, 3>(
  ...
  inline Matrix<T, 3> ToMatrix() {
    ...
    return Matrix<T, 3>(1 - 2*(y2 + z2), 2*(xy + sz), 2*(xz - sy), 2*(xy - sz),
      1 - 2*(x2 + z2), 2*(sx + yz), 2*(sy + xz), 2*(yz - sx), 1 - 2*(x2 + y2));
  }
  ...
  static Quaternion<T> FromEulerAngles(const Vector<T, 3>& angles) {
    const Vector<T, 3> halfAngles(
      .5 * angles[0], .5 * angles[1], .5 * angles[2]);
  ...
  static inline Quaternion<T> Slerp(
    ...
    if(q1.s_ * q2.s_ + Vector<T, 3>::DotProduct(q1.v_, q2.v_) > .999999)
      return Quaternion<T>(
