...
void Initialize_Test(const T& precision) {
  ...
  goomath::Matrix<T, d> matrix_splat(3.1);
  ...
  goomath::Matrix<T, d> matrix_arr(x);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
  ...
  // equals the old one.
  goomath::Matrix<T, d> matrix_copy(matrix_arr);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
  ...
  matrix_copy = matrix_copy - goomath::Matrix<T, d>(1);
  EXPECT_NE(matrix_copy(0, 0), matrix_arr(0, 0));
  // This will test creation of the identity matrix.
  goomath::Matrix<T, d> identity(goomath::Matrix<T, d>::Identity());
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
...
void InitializePerDimension_Test(const T& precision) {
  goomath::Matrix<T, 2> matrix_f2x2(4.5, 3.4, 2.6, 9.8);
  ...
  goomath::Matrix<T, 3> matrix_f3x3(
  ...
  goomath::Matrix<T, 4> matrix_f4x4(
...
void AddSub_Test(const T& precision) {
  ...
  goomath::Matrix<T, d> matrix1(x1), matrix2(x2);
  // This will test the negation of a matrix and verify that each element
  // is negated.
  goomath::Matrix<T, d> neg_mat1(-matrix1);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
  ...
  goomath::Matrix<T, d> matrix_add(matrix1 + matrix2);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
  ...
  goomath::Matrix<T, d> matrix_sub(matrix1 - matrix2);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
...
void Mult_Test(const T& precision) {
  ...
  goomath::Matrix<T, d> matrix_mults(matrix1 * 1.1);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
  ...
  goomath::Vector<T, d> vector(v);
  // This will test matrix vector multiplication and verify that the resulting
  // vector is matematically correct.
  goomath::Vector<T, d> vector_multv(matrix1 * vector);
  for (int i = 0; i < d; ++i) {
    T v1[d];
    for (int k = 0; k < d; ++k) v1[k] = matrix1(i, k);
    goomath::Vector<T, d> vec1(v1);
    T dot = goomath::Vector<T, d>::DotProduct(vec1, vector);
  ...
  goomath::Matrix<T, d> matrix_multm(matrix1 * matrix2);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < d; ++j) {
      T v1[d], v2[d];
      for (int k = 0; k < d; ++k) v1[k] = matrix1(i, k);
      for (int k = 0; k < d; ++k) v2[k] = matrix2(k, j);
      goomath::Vector<T, d> vec1(v1), vec2(v2);
      T dot = goomath::Vector<T, d>::DotProduct(vec1, vec2);
...
void OuterProduct_Test(const T& precision) {
  ...
  goomath::Matrix<T, d> matrix(
    goomath::Matrix<T, d>::OuterProduct(vector1, vector2));
...
void Inverse_Test(const T& precision) {
  ...
  goomath::Matrix<T, d> matrix(x);
  goomath::Matrix<T, d> inverse_matrix(matrix.Inverse());
  goomath::Matrix<T, d> identity_matrix(matrix * inverse_matrix);
